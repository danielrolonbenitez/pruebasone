<h1><?php echo h(__('Editar imágenes')); ?></h1>
...
<form action="<?php echo Router::url('/pages/gallery_picture_new'); ?>" method="post" enctype="multipart/form-data">
...
<div class="gallery" id="gallery">
	...
	<a href="<?php echo Router::url('/pages/gallery_picture_crud/' . sprintf('%d', $picture['Picture']['id'])); ?>" class="iframe" data-id="<?php printf('%d', $picture['Picture']['id']); ?>">
		<span class="order"></span>
		<span class="nosort">
			<span class="align-hack"></span>
			<span class="img">
				<img src="<?php echo Router::url('/'); ?>files/<?php echo h($picture['Picture']['file_name']); ?>" alt="">
			</span>
		</span>
	</a>
	<?php endforeach; ?>
...
$("#gallery").sortable({
	...
	"stop": function() {
		...
		$.ajax({
			"url": "<?php echo addslashes(Router::url('/pages/gallery_reorder', true)); ?>",
			"type": "POST",
			"data": {
				"id": ids
			}
...
function deletePicture(id) {
	...
	$.ajax({
		"url": "<?php echo addslashes(Router::url('/pages/gallery_picture_delete', true)); ?>",
		"type": "POST",
		"data": {
			"id": id
		}
